...
class GraphConstruct(nn.Module):
    ...
    def __init__(self, scale, indexer, k, patchsize, stride, padding=None, with_diff = True, with_score = False, training = True):
        r"""
    ...
    def forward(self, xe, ye):
        ...
        # print('I1222222222222222222222222',I)
        ...
        diff_patch1 = diff_patch
        score_k1 = score_k
        # print('diff_patch1diff_patch1diff_patch1diff_patch1diff_patch', diff_patch1)
        # print('idx_kidx_kidx_kidx_kidx_kidx_kidx_k', idx_k)
        if self.with_diff:
            ...
            #
            # diff_patch -> b,m,e,k      b m1*m2 ce e1*e2 k
            diff_patch1 = abs(diff_patch1.view(b, m, ce, e1 * e2, k))
            diff_patch1 = torch.sum(diff_patch1, dim=3, keepdim=True)
            diff_patch1 = diff_patch1.expand(b, m, ce, e1 * e2, k)

            # diff_patch: b m ce e1*s*e2*s k; e1==p1, e2==p2;
            # diff_patch -> b k ce e1*s*e2*s m
            diff_patch1 = diff_patch1.permute(0, 4, 2, 3, 1).contiguous()
            # diff_patch -> b k*ce e1*s e2*s m1 m2
            diff_patch1 = diff_patch1.view(b, k * ce, e1, e2, m1, m2)
            padding_sr1 = [p for p in padding]
            # z_sr -> b k*c_y H*s W*s
            diff_patch1 = ops.patch2im(diff_patch1, self.patchsize, self.stride, padding_sr1)
            diff_patch1 = diff_patch1.contiguous().view(b, k * ce, H, W)
        ...
        if self.with_score:
            ...
            score_k1 = score_k1.permute(0, 2, 1).contiguous().view(b, k, 1, 1, m1, m2)
            score_k1 = score_k1.view(b, k, 1, 1, m1, m2).expand(b, k, e1 * self.scale, e2 * self.scale, m1, m2)
            padding_sr1 = [p for p in padding]
            score_k1 = ops.patch2im(score_k1, self.patchsize , self.stride, padding_sr1)
            score_k1 = score_k1.contiguous().view(b, k, H, W)
        ...
        # print('~~~~~~~~~diff_patch1diff_patch1diff_patch1diff_patch1', diff_patch1)
        return score_k1, idx_k, diff_patch1
...
class GraphAggregation(nn.Module):
    ...
    def forward(self, y, yd, idx_k):
        ...
        ###############
        y_patch1 = y_patch1.permute(0, 4, 5, 1, 2, 3).contiguous().view(b, m, f)
        yd_patch1 = yd_patch1.permute(0, 4, 5, 1, 2, 3).contiguous().view(b, n, f)

        # Get nearest neighbor volumes
        # z_patch -> b m1*m2 c*p1*p2 k
        z_patch1 = self.aggregation(yd_patch1, idx_k)

        # Adaptive_instance_normalization
        if WITH_ADAIN_NROM:
            reduce_scale = self.scale ** 2
            print('ITH_ADAIN_NROM')
            y_patch_norm1 = y_patch1.view(b, m, c , p1 * p2)
            z_patch_norm1 = z_patch1.view(b, m, c, p1 * p2, k)
            z_patch1 = net_utils.adaptive_instance_normalization(y_patch_norm1, z_patch_norm1).view(*z_patch1.size())

        # z_patch -> b k*c p1 p2 m1 m2
        z_patch1 = z_patch1.permute(0, 3, 2, 1).contiguous()

        z_patch_sr1 = z_patch1.view(b, k, c, 1, 1, p1, p2, m1, m2).permute(0, 1, 2, 5, 3, 6, 4, 7, 8).contiguous()
        z_patch_sr1 = z_patch_sr1.view(b, k * c, p1, p2, m1, m2)
        padding_sr1 = [p for p in padding]
        # z_sr -> b k*c_y H*s W*s
        z_sr1 = ops.patch2im(z_patch_sr1, self.patchsize, self.stride , padding_sr1)
        z_sr1 = z_sr1.contiguous().view(b, k * c, H, W)
...
class Graph(nn.Module):
    ...
    Graph Construction
    """
    def __init__(self, scale, k=5, patchsize=3, stride=1, window_size=7, in_channels=16, embed_ch=32,embed_out=32,  WITH_DIFF = True,
                 WITH_SCORE= False, embedcnn=None, training=True):
        r"""
...
class GCNBlock(nn.Module):
    ...
    Graph Aggregation
    """
    def __init__(self, nplanes_in, scale, k=5, patchsize=3, stride=1, diff_n=24):
        r"""
    ...
    def forward(self,y_d, y, idx_k, diff_patch):
        ...
        knn_hr = self.graph_aggregate(y, y_d, idx_k)
        # knn_hr torch.Size([1, 1280, 80, 80])

        # for diff socre
        knn_hr = self.weight_edge(knn_hr, diff_patch)
